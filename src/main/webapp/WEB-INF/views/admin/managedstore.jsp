...
<head>
    ...
    <link rel="stylesheet" href="../../../css/adminmanagedstores.css" />
    ...
    <div class="wrapper">
        ...
        <div class="main">
            <div>
                ...
                <div class="content">
                    <p class="menuName">가게 승인</p>
                    <form method="get" action="<c:url value='/admin/managedstore'/>">
                    <div class="usersFilter">
                        <select class="selectBox" name="s_type">
                            <option value="store_id" ${store.s_type == 'store_id' ? 'selected' : ''}>ID</option>
                            <option value="store_name" ${store.s_type == 'store_name' ? 'selected' : ''}>가게명</option>
                            <option value="owner_name" ${store.s_type == 'owner_name' ? 'selected' : ''}>대표명</option>
                        </select>
                    </div>
                    <div class="filterValue">
                        <input class="searchBox" type="text" name="s_value" value="${store.s_value != null ? store.s_value : ''}">
                        <div class="magnifying">
                            <input class="magnifyingButton" type="submit" value=""> 
                        </div>
                    </div>
                    </form>
                    <form method="post" action="<c:url value='/admin/updatestore'/>">
                    <div class="updateDiv">
                        ...
                        <table border="1">
                            ...
                            <c:forEach var="vo" items="${map.list}">
                                <tr>
                                    ...
                                    <td>
                                        <input type="hidden" name="s_store_id" value="${vo.store_id}">   
                                        <input type="checkbox" name="s_confirmed" value="${vo.store_id}" ${vo.confirmed == 'Y' ? 'checked' : ''}> 
                                    </td>
                                </tr>
                            </c:forEach>
                        </table>
                    ...
                    <div class="page">
                        <ul class='paging'>
                            ...
                            <c:forEach var="p" begin="${map.startPage}" end="${map.endPage}">
                                ...
                                <li><a href='managedstore?page=${p}&s_type=${store.s_type}&s_value=${store.s_value}'>${p}</a></li>
                                </c:if>
                            </c:forEach>
                            <c:if test="${map.isNext }">
                                <li><a href="managedstore?page=${map.endPage+1 }&s_type=${store.s_type}&s_value=${store.s_value}"> >> </a></li>
                            </c:if>
                        </ul>
                    </div>
                </div>
